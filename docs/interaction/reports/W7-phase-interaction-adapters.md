...
- Incorporated the Orchestrator's Unity 6000.5.6f1 evidence. All eleven PlayMode leaves passed individually, including fake-null recovery, disabled/configure ownership, per-event A-to-B detachment, audio, hint, planned-key restoration, terminal snapshots, and the real runtime subscription chain. After the preceding product/fixture fixes, the full EditMode rerun produced 21 pass / 3 fail; root then reran the three formerly failing leaves (`CleanActive`, `IsolatedDirty`, and `Strip`) against that fix and reported 3/3 green. This final test-only robustness follow-up adds cleanup-annotation, unique malformed-token, active-scene, and root-count assertions after that targeted Unity result. The current source therefore remains pending Unity execution; this report does not claim a post-follow-up 24/24 result.
...
This final test-robustness follow-up changed exactly these existing files (no `.meta` or
...
Latest test-robustness TDD/static gate:

```text
ROOT_UNITY_PREVIOUSLY_RED_TARGETED_RERUN=3 passed, 0 failed, 3 total (real green before this follow-up)
ROOT_UNITY_POST_ROBUSTNESS_CHANGE=not run; no 24/24 claim

RED_CLEAN_PRIMARY_HAS_NO_CLEANUP_ASSERT=0
RED_FIXED_MALFORMED_Z_TOKEN=1
RED_UNIQUE_MALFORMED_TOKEN=0
RED_DIRTY_ACTIVE_ASSERT=0
RED_DIRTY_ROOT_COUNT_ASSERT=0
RED_PROCESS_EXIT=1

GREEN_CLEAN_PRIMARY_HAS_NO_CLEANUP_ASSERT=1
GREEN_FIXED_MALFORMED_Z_TOKEN=0
GREEN_UNIQUE_MALFORMED_TOKEN=1
GREEN_MALFORMED_LENGTH_ASSERT=1
GREEN_MALFORMED_NONHEX_ASSERT=1
GREEN_DIRTY_ACTIVE_ASSERT=1
GREEN_DIRTY_ROOT_COUNT_ASSERT=1
...
`%LOCALAPPDATA%\Temp\signvr-w7-test-robustness-20260826-082437`. The expected
primary fixture failure now proves that its `Data` contains no cleanup-failure
annotation; the malformed ownership token retains 31 random hexadecimal
characters and appends one non-hexadecimal `z`; and the dirty-user fixture now
proves both active-scene identity and unchanged root count after the nested
fixture returns.

Immediately preceding temporary-scene-copy/NUnit-array TDD/static gate:
...
Twenty-four Unity-facing EditMode NUnit methods and eleven PlayMode `[UnityTest]` methods source-compiled successfully. The EditMode suite additionally covers temporary-copy setup from zero W7 wiring with exact left/right test hand roots, the five strict marker/token/path negative cases, a per-run unique 32-character malformed token, canonical loaded/dirty InteractionLab preservation, dirty in-memory and clean saved temporary-copy preservation, clean saved previous-active-scene preservation across success/failure/guard paths, cleanup-free primary failure propagation, dirty active-scene/root-count restoration, injected creation cleanup, zero public result forwarding in non-play completion/GiveUp, configured-adapter fail-closed behavior before a W1 snapshot/for a non-current phase/under disabled authority, the standalone-adapter exception, independent cleanup after a simulated close failure, exact generated ownership, component-free generated-state counting, and authored Behaviour/Collider/Rigidbody/pose restoration through the Editor-only pre-destroy contract. The exact PlayMode methods are `RuntimeSubscriptionChainDeliversOnceAcrossLifecycle`, `AvailabilitySubscribersStayClosedWhileDisabled`, `ConfigureFailureIsAtomicAndInputOwnershipMovesToB`, `ResultPresentersUseRealPlayModeSubscriptions`, `RuntimeReenableHonorsTerminalSnapshotAndRebuildsChest`, `HintPresenterRebuildsAuthorityAcrossLifecycle`, `PresenterConfigureIsAtomicAndTransfersOutputOwnership`, `FeedbackPresenterStopsOnlyItsOwnedAudioOutput`, `PlannedKeyReleaseRestoresAuthoredPoseAndMotion`, `AdapterRecoversAfterDestroyedCoordinatorReplacement`, and `EachSubscribedEventMovesFromPublisherAToBExactlyOnce`. Root Unity executed these eleven leaves and reported 11/11 green. Root also reported the three previously RED EditMode leaves 3/3 green before the current assertion-only follow-up. Current source has not received a new full or targeted Unity run, so no post-follow-up 24/24 claim is made. The tests use real `Application.isPlaying` CLR subscriptions, public event counters, test-only read-only per-event diagnostics, distinct A/B outputs, real runtime audio, dynamic/kinematic Rigidbody outcomes, and destroyed Unity-object replacement rather than private subscribed flags or event backing fields.
...
5. Root has already rerun the three formerly failing leaves (`CleanActiveSavedSceneSurvivesSuccessfulAndFailedFixtures`, `IsolatedSetupPreservesDirtyUnsavedUserScene`, and `StripUsesExactOwnershipAndRestoresAuthoredRuntimeState`) and reported 3/3 green. After integrating this assertion-only follow-up, rerun `CleanActiveSavedSceneSurvivesSuccessfulAndFailedFixtures`, `TestOwnedSceneGuardRejectsMissingDuplicateAndMalformedOwners`, and `IsolatedSetupPreservesDirtyUnsavedUserScene`. Confirm the expected primary exception has no `W7FixtureCleanupFailures` data, repeated/interrupted runs use different malformed-token asset paths, and the dirty temporary user scene remains active with an unchanged root count. A final full 24-leaf EditMode regression is preferred, but none is claimed by this worker. Root already recorded PlayMode 11/11 green; this change does not alter runtime code.
