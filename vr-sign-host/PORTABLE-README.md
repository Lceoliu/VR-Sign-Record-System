...
1. 另一录制组的 Host 可以继续运行；本组已使用独立端口和设备过滤，不会与它争抢 Quest。
...
- Quest：先将包含独立端口配置的最新 APK Build And Run 到本组 Quest 一次。之后打开应用，Host 每 3 秒自动发现 Quest，并在首次发现时把上传地址刷新为新电脑 IP；迁移主机时无需再次烧录。
