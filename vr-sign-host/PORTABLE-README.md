...
5. 第一次启动会弹出 Windows 管理员确认，用于开放局域网 TCP 8000 和 UDP 5005。确认后以后不再弹出。
6. 浏览器会自动打开 `http://127.0.0.1:8000`。如果没有自动打开，手动输入该地址。
...
- 提示端口 8000 被占用：关闭旧的 SignVR 窗口或占用该端口的程序，再重新双击 BAT。
- 网页看不到 Quest：确认旧 Host 已关闭、Quest 应用正在前台、两台设备都连接 `RhythMo`，然后在网页点击扫描。
- Quest 能显示但 Pose 不上传：确认首次启动时已经允许管理员弹窗，并检查 Windows 防火墙中存在 `SignVR Host HTTP 8000` 和 `SignVR Host UDP 5005`。
