...
{
    /// <summary>
    /// Interaction namespace counterpart of the Recorder's sprite-free rounded
    /// prompt surface, kept Recorder-state-free for InteractionLab validation.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class InteractionRoundedRectangleGraphic : MaskableGraphic
    {
        [SerializeField]
        [Min(0f)]
        ...
        protected override void OnPopulateMesh(VertexHelper vertexHelper)
        {
            ...
            for (int corner = 0; corner < 4; corner++)
            {
                ...
                {
                    float radians = (startDegrees +
                        (float)segment / segments * 90f) * Mathf.Deg2Rad;
            ...
            for (int index = 0; index < perimeterCount; index++)
            {
                vertexHelper.AddTriangle(
                    0,
                    index + 1,
                    ((index + 1) % perimeterCount) + 1
                );
            }
        }

        private static Vector2 CornerCenter(
            Rect rect,
            float radius,
            int corner)
        {
